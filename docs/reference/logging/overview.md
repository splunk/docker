...
logging driver options.
  ...
  - `splunk-data-*` optional, specify additional data for each log entry which will be added to the `data' object in the `event`. The suffix becomes the key i.e. `splunk-data-category=foo` will use a key of category and a value of foo.
...
`docker run --log-driver=splunk \
    ...
    --log-opt splunk-data-category="Category 1"
